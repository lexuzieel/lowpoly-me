...
with sync_playwright() as p:
    ...
    if os.environ.get("BROWSER") == "firefox":
        b = p.firefox.launch(firefox_user_prefs={"media.autoplay.default": 0, "webgl.force-enabled": True})
    else:
    ...
    if "--play" in sys.argv:
        pg.click("#card")
        t = next((float(a.split("=")[1]) for a in sys.argv if a.startswith("--at=")), 17.3)
        pg.evaluate(f"document.getElementById('song').currentTime = {t}")
